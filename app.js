...
function createTab(url = HOME) {
  const id = Date.now();
  const tab = { id, url, history: [url], index: 0 };
  tabs.push(tab);
  activeTab = id;
  renderTabs();
  navigateTo(url);
...
backBtn.onclick = () => { const tab = tabs.find(t => t.id === activeTab); if (tab.index > 0) { tab.index--; navigateTo(tab.history[tab.index], false); } };
forwardBtn.onclick = () => { const tab = tabs.find(t => t.id === activeTab); if (tab.index < tab.history.length - 1) { tab.index++; navigateTo(tab.history[tab.index], false); } };
